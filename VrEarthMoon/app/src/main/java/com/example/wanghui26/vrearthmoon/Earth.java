...
public class Earth {
    ...
    private int muTextureDayHandle;
    private int muTextureNightHandle;
    ...
    private void initVertexData(float r) {
        ...
        Log.e("Earth", "vCount = " + vCount + ", alVertix.size() = " + alVertix.size() + ", vertices.length = " + vertices.length);
    ...
    private void initShader(MySurfaceView mySurfaceView) {
        ...
        muLightLocationSunHandle = GLES20.glGetUniformLocation(mProgram, "uLightLocationSun");
        maPositionHandle = GLES20.glGetAttribLocation(mProgram, "aPosition");
        maTexCoorHandle = GLES20.glGetAttribLocation(mProgram, "aTexCoor");
        maNormalHandle = GLES20.glGetAttribLocation(mProgram, "aNormal");

        muTextureDayHandle = GLES20.glGetUniformLocation(mProgram, "sTextureDay");
        muTextureNightHandle = GLES20.glGetUniformLocation(mProgram, "sTextureNight");
    }
    ...
    public void drawSelf(int texId1, int texId2) {
        ...
        GLES20.glVertexAttribPointer(maNormalHandle, 3, GLES20.GL_FLOAT, false, 3 * 4, mVertexBuffer); // 4

        GLES20.glEnableVertexAttribArray(maNormalHandle);
        GLES20.glEnableVertexAttribArray(maPositionHandle);
        GLES20.glEnableVertexAttribArray(maTexCoorHandle);

        GLES20.glActiveTexture(GLES20.GL_TEXTURE0);     //指定哪一个纹理单元被置为活动状态。
        // texture必须是GL_TEXTUREi之一，其中0 <= i < GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS，初始值为GL_TEXTURE0。
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, texId1);
        GLES20.glActiveTexture(GLES20.GL_TEXTURE1);
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, texId2);
        GLES20.glUniform1i(muTextureDayHandle, 0);
        GLES20.glUniform1i(muTextureNightHandle, 1);
